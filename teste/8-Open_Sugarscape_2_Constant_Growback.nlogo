...
to solicita_cartorio
  let path2 "important/request_new_agent2.txt"
  file-open path2
...
to send_agent_to_model
  if (send_agents)
  [
    let path2 "important/send_agent2.txt"
    file-open path2
...
to check_new_agent
  if (file-exists? "important/receive_agent2.txt")
  [
    file-open "important/receive_agent2.txt"
    ;print "---"
    ...
    print (count turtles)
    file-close
    ;carefully [ file-delete "receive_agent.txt" ] [ print error-message ]
    let deleted false
    ...
    [
      carefully
      [
        file-delete "important/receive_agent2.txt"
        print "file deleted"
        set deleted true
        set qtd_received_agents (qtd_received_agents + 1)
      ]
      [
        print error-message
      ]
...
to print_alive_agents
  foreach sort-on [agent_id] turtles [ the-turtle -> ask the-turtle [ print agent_id ] ]
end
...
<experiments>
  <experiment name="experiment1" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>print_agent_random</final>
