// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_580]) & (Ans =.. [NS,F,T,NLA])))).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_538,WksName,_539).
joinedWsp(cobj_2,mining,"/main/mining")[artifact_id(cobj_1),artifact_name(session_551),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joinedWsp(cobj_0,main,"/main")[artifact_id(cobj_1),artifact_name(session_551),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_540).
last_dir(down).
focusing(cobj_3,m2view,"mining.MiningPlanet",cobj_2,mining,"/main/mining")[artifact_id(cobj_4),artifact_name(body_551),percept_type(obs_prop),source(percept),workspace("/main/mining",cobj_2)].
teste(0.04696754528733771).
...
@pteste2[atomic,source(self),url("file:src/agt/miner3.asl")] +initial_print1 <- .print("Hello there. I'm saving myself to leave the simulation"); ?agent_id(V0); ?path(V1); .random(R); +teste(R); ?teste(R); .print("R: ",R); .abolish(agent_id(_541)); .abolish(path(_542)); ?sugar(B0); ?metabolism(B1); ?vision(B2); .print("Sugar ",B0); .print("Metabolism ",B1); .print("Vision ",B2); .abolish(sugar(_543)); .abolish(metabolism(_544)); .abolish(vision(_545)); .my_name(X); .concat("src/agt/list/",X,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(V0,V1,X,B0,B1,B2)); .send(killer_agent,untell,kill(V0,V1,X,B0,B1,B2)).
@p__112[source(self),url("file:src/agt/miner3.asl")] +free : (gsize(_546,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__115[source(self),url("file:src/agt/miner3.asl")] +!go_near(X,Y) : free <- -near(_547,_548); -last_dir(_549); !near(X,Y).
...
@p__121[source(self),url("file:src/agt/miner3.asl")] +!next_step(X,Y) : not (pos(_550,_551)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/miner3.asl")] +!init_handle(Gold) : .desire(near(_552,_553)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_554,_555)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/miner3.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pteste[atomic,source(self),url("file:src/agt/miner3.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_556,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +initial_print1.
...
@p__128[source(self),url("file:src/agt/miner3.asl")] +!ensure(pick,_557) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__129[source(self),url("file:src/agt/miner3.asl")] +!ensure(drop,_558) : (carrying_gold & (pos(X,Y) & depot(_559,X,Y))) <- drop.
...
@p__132[source(self),url("file:src/agt/miner3.asl")] +!choose_gold : not (gold(_560,_561)) <- -+free.
@p__133[source(self),url("file:src/agt/miner3.asl")] +!choose_gold : gold(_562,_563) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_564,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__137[source(self),url("file:src/agt/miner3.asl")] +!calc_gold_distance([_565|R],RD) <- !calc_gold_distance(R,RD).
@p__138[source(self),url("file:src/agt/miner3.asl")] +end_of_simulation(S,_566) <- .drop_all_desires; .abolish(gold(_567,_568)); .abolish(picked(_569)); -+free; .print("-- END ",S," --").
...
@p__107[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_535).
...
@p__109[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_536),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_537); .print("joinned workspace ",FullW).
