// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_1032]) & (Ans =.. [NS,F,T,NLA])))).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_990,WksName,_991).
...
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_992).
last_dir(down).
...
@pteste2[atomic,source(self),url("file:src/agt/miner3.asl")] +initial_print1 <- .print("Hello there. I'm saving myself to leave the simulation"); ?agent_id(V0); ?path(V1); .random(R); +teste(R); ?teste(R); .print("R: ",R); .abolish(agent_id(_993)); .abolish(path(_994)); ?sugar(B0); ?metabolism(B1); ?vision(B2); .print("Sugar ",B0); .print("Metabolism ",B1); .print("Vision ",B2); .abolish(sugar(_995)); .abolish(metabolism(_996)); .abolish(vision(_997)); .my_name(X); .concat("src/agt/list/",X,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(V0,V1,X,B0,B1,B2)); .send(killer_agent,untell,kill(V0,V1,X,B0,B1,B2)).
@p__176[source(self),url("file:src/agt/miner3.asl")] +free : (gsize(_998,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__179[source(self),url("file:src/agt/miner3.asl")] +!go_near(X,Y) : free <- -near(_999,_1000); -last_dir(_1001); !near(X,Y).
...
@p__185[source(self),url("file:src/agt/miner3.asl")] +!next_step(X,Y) : not (pos(_1002,_1003)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/miner3.asl")] +!init_handle(Gold) : .desire(near(_1004,_1005)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_1006,_1007)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/miner3.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pteste[atomic,source(self),url("file:src/agt/miner3.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_1008,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +initial_print1.
...
@p__192[source(self),url("file:src/agt/miner3.asl")] +!ensure(pick,_1009) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__193[source(self),url("file:src/agt/miner3.asl")] +!ensure(drop,_1010) : (carrying_gold & (pos(X,Y) & depot(_1011,X,Y))) <- drop.
...
@p__196[source(self),url("file:src/agt/miner3.asl")] +!choose_gold : not (gold(_1012,_1013)) <- -+free.
@p__197[source(self),url("file:src/agt/miner3.asl")] +!choose_gold : gold(_1014,_1015) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_1016,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__201[source(self),url("file:src/agt/miner3.asl")] +!calc_gold_distance([_1017|R],RD) <- !calc_gold_distance(R,RD).
@p__202[source(self),url("file:src/agt/miner3.asl")] +end_of_simulation(S,_1018) <- .drop_all_desires; .abolish(gold(_1019,_1020)); .abolish(picked(_1021)); -+free; .print("-- END ",S," --").
...
@p__171[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_987).
...
@p__173[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_988),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_989); .print("joinned workspace ",FullW).
