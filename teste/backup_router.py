...
# json_dead_agents_file = ["important/send_agent.txt", "important/send_agent2.txt"]
# general_dead_agents_file = ["important/general_send_agent.txt"][0]
# json_model_input_file = ["important/receive_agent.txt", "important/receive_agent2.txt"]
# output_agents = ["important/output.txt"][0]
# router_type = "random"
...
def main(args=None):
    ...
    for x in range(len(json_model_input_file)):
         model_list.append(["model"+str(x), json_model_input_file[x], json_dead_agents_file[x], 0])
        ...
        if router_type == "random":
            if(os.path.isfile(general_dead_agents_file)):
                if(os.stat(general_dead_agents_file).st_size > 0):
                    ...
                    if(len(dead_agents_list) > 0):
                        print("Escolhendo um agente pra reviver")
                        # print('dead_agents_list: [%s]' % ', '.join(map(str, dead_agents_list)))
                        # print("---")
                        # print("dead0: "+dead_agents_list[0])

                        chosen_agent_original = random.choice(dead_agents_list)
                        chosen_agent = chosen_agent_original.strip('\n')
                        print("Agent chosen:")
                        print (chosen_agent)
                        ...
                        dead_agents_list.remove(chosen_agent_original)
                        ...
                        for model in model_list:
                            print("Qtd de agentes enviados pro modelo "+str(model[0])+" : "+str(model[3]))
                        time.sleep(1)
                else:
    ...
    print("Último agente adicionado na fila: " + ultimo_agente_adicionado_na_fila)
    print("***********************")
    # print(dead_agents_list)
    for agent in dead_agents_list:
        print(agent)
        print("\n")
    print("***********************")
    print("acabou")
