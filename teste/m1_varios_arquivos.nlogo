...
to new_solicita_cartorio
  ...
  let teste py:runresult (word "solicita_cartorio('" ("m1") "'" "," (1) ", " (400) ")")
...
to send_agent_to_model
  ...
  [
    ...
    let path2 (word "important/1/out" (agent_id) ".txt")
    file-open path2
...
to new_check_new_agent
  ...
  let teste py:runresult (word "teste_recebimento('" ("m1") "')")
...
to check_new_agent
  file-close-all
  let original_file "important/list_of_agents_1.txt"
  let to_delete []
  let files_to_check []
...
;  ]
  ...
  print("AGENTES VIVOS: ")
  print_alive_agents
;  export-output "important/m1.txt"
  print("---------- FIM!")
end
