// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_7359]) & (Ans =.. [NS,F,T,NLA])))).
...
@p__699[source(self),url("file:src/agt/regular_agent.asl")] +initial_print <- .print("I'm a regular agent, from the model running"); ?previous_0(V0); ?previous_1(V1); ?previous_2(V2); .print("Vindo do agente na criação, previous_0: ",V0); .print("Vindo do agente na criação, previous_1: ",V1); .print("Vindo do agente na criação, previous_2: ",V2).
@plan1[atomic,source(self),url("file:src/agt/regular_agent.asl")] +initial_print2 <- .print("Hello there. I'm a saver agent"); ?previous_0(V0); ?previous_1(V1); ?previous_2(V2); .abolish(previous_0(_7343)); .abolish(previous_1(_7344)); .abolish(previous_2(_7345)); ?sugar(B0); ?metabolism(B1); ?vision(B2); .abolish(sugar(_7346)); .abolish(metabolism(_7347)); .abolish(vision(_7348)); .my_name(X); .concat("src/agt/list/",X,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(V0,V1,V2,X,B0,B1,B2)); .send(killer_agent,untell,kill(V0,V1,V2,X,B0,B1,B2)).
...
@p__701[source(self),url("file:src/agt/regular_agent.asl")] +anser_previous(X) <- .print("I've received a message to kill agent ",X); .print("Killing agent ",X).
